...
void BrushESC775_solve_motor_status(INTF_Motor_HandleTypeDef *m, uint8_t *data) {
    ...
    ((BrushESC775_ResDataTypeDef *) m->private_data)->current = uint_to_float(controller_status.data.current, -5, 5,
                                                                              16);
    ((BrushESC775_ResDataTypeDef *) m->private_data)->cpu_tmp = uint_to_float(controller_status.data.cpu_tmp, 0, 1000,
                                                                              16);
...
void BrushESC775_CAN_CallBack(void *message, BusSubscriberHandle_t subscriber) {
    ...
    if (msg->can_id != m->motor_id + BrushESC775_CONTROLLER_CAN_P1_Base &&
        msg->can_id != m->motor_id + BrushESC775_CONTROLLER_CAN_P2_Base) {
        return;
    }
    if (msg->can_id == m->motor_id + BrushESC775_CONTROLLER_CAN_P1_Base) {
...
static void print_info(ITNF_ManagerdMotor_HandleTypedef *manager, char *buff, uint16_t len) {
    ...
    snprintf(buff, len, "Vm:%.4f Current:%.4f Cpu_tmp:%.4f Fuel:%.4f", priv->Vm, priv->current, priv->cpu_tmp,
             m->real_torque);
}
